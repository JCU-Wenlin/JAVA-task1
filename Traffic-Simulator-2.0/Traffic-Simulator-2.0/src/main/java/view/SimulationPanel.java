...
public class SimulationPanel extends JPanel {
    ...
    private void createVehicle() {
        ...
        switch (randomVehicle) {
            case 0:
                vehicles.add(new Car(Integer.toString(cycle), roads.get(0), EAST));
                break;
            case 1:
                vehicles.add(new Bus(Integer.toString(cycle), roads.get(0), EAST));
                break;
            case 2:
                vehicles.add(new Motorbike(Integer.toString(cycle), roads.get(0), EAST));
                break;
        }
    }
...
//                vehicle.move();
                vehicle.printStatus();
//                if (vehicle.getTailPosition() + vehicle.getLength() + vehicle.getSpeed() >= vehicle.getCurrentRoad().getLength() && vehicle.getCurrentRoad().getConnectedRoads().isEmpty() && (vehicle.getSpeed() == 0)) {
